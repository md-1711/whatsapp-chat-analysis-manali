...
def response_time_analysis(selected_user, df):
    ...
    df['response_time'] = (df['date'] - df['prev_date']).fillna(pd.Timedelta(seconds=0))
...
def media_types_shared_analysis(selected_user, df):
    ...
    media_types = df['message'].apply(lambda x: 'image' if 'image omitted' in x.lower() else 'video' if 'video omitted' in x.lower() else 'other' if 'media omitted' in x.lower() else 'text')
    media_types_count = media_types.value_counts()
    return media_types_count
