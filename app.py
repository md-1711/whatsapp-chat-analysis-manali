...
if uploaded_file is not None:
    ...
    if st.sidebar.button("Show Analysis"):
        ...
        # Stats Area
        num_messages, words, num_media_messages, num_links = utils.generate_stats(selected_user, df)
        ...
        timeline = utils.monthly_activity_timeline(selected_user, df)
        fig, ax = plt.subplots()
        ...
        # Finding the busiest users in the group (Overall only)
        if selected_user == 'Overall':
            st.title('Most Busy Users')
            x, new_df = utils.top_active_users(df)
            fig, ax = plt.subplots()
